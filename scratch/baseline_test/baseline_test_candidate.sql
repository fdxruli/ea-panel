...
BEGIN
    year_month := TO_CHAR(NOW(), 'MMYY'); -- Formato MesA√±o, ej: '0925'

    -- Bucle que se ejecutar√° hasta encontrar un c√≥digo √∫nico
    WHILE NOT is_unique LOOP
        -- Genera un n√∫mero aleatorio entre 100 y 999
        random_suffix := LPAD(FLOOR(RANDOM() * 900 + 100)::INT::TEXT, 3, '0');
        
        -- Construye el c√≥digo potencial
        new_code := 'EA-' || year_month || '-' || random_suffix;
        
        -- Verifica si este c√≥digo ya existe
        PERFORM 1 FROM orders WHERE order_code = new_code;
        
        -- Si no se encontr√≥, el c√≥digo es √∫nico
        IF NOT FOUND THEN
            is_unique := TRUE;
        END IF;
    END LOOP;

    -- Asigna el c√≥digo √∫nico al nuevo pedido
...
AS $function$
BEGIN
    -- CORRECCI√ìN: Remover CONCURRENTLY porque la vista tiene solo 1 fila
...
BEGIN
    -- 1. Comprobar la condici√≥n que pediste:
    -- ¬øEl nuevo estado es 'cancelado' Y el estado antiguo era 'pendiente'?
...
BEGIN
  PERFORM
    -- üëá CORRECCI√ìN: Usar net.http_post
    net.http_post(
      url:='https://xvstqhvooabljhhfmuas.functions.supabase.co/send-order-notification', -- Especificar nombre del par√°metro
      body:=jsonb_build_object( -- Especificar nombre del par√°metro
        'record', to_jsonb(NEW),
        'old_record', to_jsonb(OLD)
      ),
      headers:='{"Content-Type": "application/json"}'::jsonb -- Especificar nombre y tipo del par√°metro
    );
...
BEGIN
    -- 1. Obtener el factor de conversi√≥n (ej: "Garrafa" -> 3500)
    ...
    ELSE
        new_avg_cost := 0; -- Evitar divisi√≥n por cero si el stock es 0
    END IF;
...
BEGIN
    ...
    IF NEW.type = 'category' AND NEW.target_id IS NOT NULL THEN
        IF NOT EXISTS (SELECT 1 FROM categories WHERE id = NEW.target_id) THEN
            RAISE EXCEPTION 'target_id debe corresponder a una categor√≠a v√°lida';
        END IF;
    END IF;
    IF NEW.type = 'product' AND NEW.target_id IS NOT NULL THEN
        IF NOT EXISTS (SELECT 1 FROM products WHERE id = NEW.target_id) THEN
            RAISE EXCEPTION 'target_id debe corresponder a un producto v√°lido';
        END IF;
    END IF;
    RETURN NEW;
END;
...
BEGIN
    -- 1. VERIFICACI√ìN Y BLOQUEO DE STOCK CONSOLIDADO
    ...
    -- Obtener el c√≥digo de orden generado por el trigger
    ...
    -- 5. RETORNO DE INFORMACI√ìN AL CLIENTE
    RETURN QUERY 
...
begin
  ...
  -- 'new' se refiere al nuevo registro que activ√≥ el trigger (el nuevo usuario)
...
BEGIN
  ...
  -- Si no hay permisos, asignar por defecto seg√∫n el rol
...
-- ==========================================
-- SUBSISTEMA REFERRAL (BASELINE HIST√ìRICO)
-- ==========================================
...
begin
  ...
  if not found then raise exception 'El c√≥digo de descuento base no fue encontrado.'; end if;
                ...
                ELSIF i > 0 THEN
                    ...
                    v_status_message :=
                        ...
                        || CASE
                            ...
                            WHEN v_days_diff = 2 THEN 'pasado maq√±ana'
                            ELSE 'el ' || v_day_name
                           END
                        || ' a las '
                        || to_char(v_future_exception.open_time, 'HH12:MI AM');
            ...
            IF v_future_regular IS NOT NULL
               AND NOT v_future_regular.is_closed THEN
                IF i = 0
                   ...
                   AND v_future_regular.open_time < v_future_\ôY›[\ãò€‹ŸW›[YHSÇà””ïSïQN¬àS“QàHHàSëóÿ›\úô[ù›[YHóŸù]\ôW‹ôY›[\ãõ‹[ó›[YHSÇàó‹›]\◊€Y\‹ÿYŸHèBà	–Ÿ\úòY»Z‹òHXúö[[‹»ﬁHH\»	¬à◊ÿ⁄\äóŸù]\ôW‹ôY›[\ãõ‹[ó›[YK	“LéìRHSI N¬àëUTìàú€€óÿùZ[€ÿöôX›
à	⁄\◊€‹[âÀêS—Kà	€Y\‹ÿYŸIÀó‹›]\◊€Y\‹ÿYŸBà
N¬àS“QàHàSÇàóŸ^\◊ŸYôàèHN¬àóŸ^W€ò[YHèBà–T—Hóÿ⁄X⁄◊Ÿ›¬à“SàSà	—€Z[ô€…¬à“SàHSà	”[ô\…¬à“SààSà	”X\ù\…¬à“Sà»Sà	”ZpÍ\ò€€\…¬à“SàSà	“ùY]ô\…¬à“SàHSà	’öY\õô\…¬à“SààSà	‘ËXòY…¬àSë¬àó‹›]\◊€Y\‹ÿYŸHèBà	–Ÿ\úòYÀàXúö[[‹»	¬à–T—Bà“SàóŸ^\◊ŸYôàHHSà	€XpÏX[òI¬à“SàóŸ^\◊ŸYôàHàSà	‹\ÿY»XpÏX[òI¬àS—H	Ÿ[	»óŸ^W€ò[YBàSëà	»H\»	¬à◊ÿ⁄\äóŸù]\ôW‹ôY›[\ãõ‹[ó›[YK	“LéìRHSI N¬àëUTìàú€€óÿùZ[€ÿöôX›
à	⁄\◊€‹[âÀêS—Kà	€Y\‹ÿYŸIÀó‹›]\◊€Y\‹ÿYŸBà
N¬àSëQé¬àSëQé¬àSëQé¬àSë”‘¬ÇàëUTìàú€€óÿùZ[€ÿöôX›
à	⁄\◊€‹[âÀêS—Kà	€Y\‹ÿYŸIÀ	—[ôY€ÿ⁄[»\›0ËHŸ\úòY»[\‹ò[Y[ùKà€€ú›[H∞Ïﬁ[[‹»‹ò\ö[‹Àâ¬à
N¬ëSë¬âù[ò›[€â¬