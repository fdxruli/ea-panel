...
const ProductCard = memo(({ product, categoryName, onToggle, onEdit, onManageImages, onManageAudience, onSelect }) => {
  ...
  const handleCardClick = (e) => {
    // Si el clic no provino de un botón de acción, abrir analítica 360
    ...
    >
      <div className={styles.imageContainer}>
        ...
        <span className={styles.imageCount}>
          {product.image_count || (1 + (product.product_images?.length || 0))} <Camera size={13} aria-hidden="true" />
        </span>
      ...
      <div className={styles.cardActions}>
        <button 
          ...
          title="Ver analítica 360°, estructura de costos y clientes"
        >
          <Eye size={14} style={{ marginRight: 4 }} /> Ver 360°
        </button>
        {hasPermission('productos.edit') && (
          <>
            ...
            <button 
              onClick={(e) => { e.stopPropagation(); onManageAudience && onManageAudience(product); }} 
              className={styles.audienceButton}
              title="Configurar público general o clientes especiales"
            >
              <Users size={14} style={{ marginRight: 4 }} /> Audiencia
            </button>
            <button 
