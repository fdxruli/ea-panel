...
export default function EditOrderModal({ order, onClose, onOrderUpdated }) {
    ...
    // ELIMINADO: const [categories, setCategories] = useState([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                // ELIMINADO: const categoriesPromise = supabase.from('categories').select('*').order('name');
                const addressesPromise = supabase.from('customer_addresses').select('*').eq('customer_id', order.customer_id);

                // MODIFICADO: Se elimina la petición de categorías
                const [productsRes, addressesRes] = await Promise.all([productsPromise, addressesPromise]);

                if (productsRes.error) throw productsRes.error;
                // ELIMINADO: if (categoriesRes.error) throw categoriesRes.error;
                if (addressesRes.error) throw addressesRes.error;
                
                setAllProducts(productsRes.data);
                // ELIMINADO: setCategories(categoriesRes.data);
    ...
    // ELIMINADO: El hook useMemo para 'visibleCategories' ha sido removido.
                        ...
                        <>
                            ...
                            <div className={`${styles.contentBody} ${activeTab === 'add' ? styles.addMode : ''}`}>
                                ...
                                <div className={styles.addProductSection}>
                                    ...
                                    {/* ELIMINADO: La sección de filtros de categoría se ha removido del JSX */}
