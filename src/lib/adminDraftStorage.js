...
const getDb = () => {
  if (typeof indexedDB === 'undefined') {
    throw new Error('ADMIN_DRAFT_STORAGE_UNAVAILABLE');
  }

  if (!dbPromise) {
    const db = new Dexie(DB_NAME);
    db.version(1).stores({ drafts: 'id, workflow, updatedAt, expiresAt, version' });
    dbPromise = db.open().catch((error) => {
      dbPromise = undefined;
      throw error;
    });
  }
...
const validateSerializable = (value, path = 'payload', seen = new WeakSet()) => {
  ...
  if (Array.isArray(value)) {
    value.forEach((item, index) => validateSerializable(item, `${path}[${index}]`, seen));
  } else {
    Object.entries(value).forEach(([key, item]) => validateSerializable(item, `${path}.${key}`, seen));
  }
  seen.delete(value);
};
...
const assertDraft = (draft) => {
  ...
  for (const field of ['id', 'workflow', 'createdAt', 'updatedAt', 'expiresAt', 'version']) {
    if (!draft[field]) throw new TypeError(`Invalid draft: missing ${field}`);
  }
...
const isExpired = (draft, now = Date.now()) => Number(draft.expiresAt) <= now;
...
export const createDraft = async ({ id = crypto.randomUUID(), workflow, payload, metadata = {}, ttlMs = DEFAULT_DRAFT_TTL_MS, now = Date.now() }) => {
  ...
  validateSerializable(metadata, 'metadata');

  const draft = {
    id,
    version: ADMIN_DRAFT_SCHEMA_VERSION,
    workflow,
    createdAt: now,
    updatedAt: now,
    expiresAt: now + ttlMs,
    payload: cloneDraft(payload),
    metadata: cloneDraft(metadata),
  };
  assertDraft(draft);

  const db = await getDb();
  await db.table(TABLE_NAME).add(draft);
...
export const getDraft = async (id, { now = Date.now(), cleanupExpired = true } = {}) => {
  if (!id) return null;
  const db = await getDb();
  const draft = await db.table(TABLE_NAME).get(id);
  if (!draft) return null;

  try {
    assertDraft(draft);
  } catch (error) {
    if (cleanupExpired) await db.table(TABLE_NAME).delete(id);
    return null;
  }
...
export const updateDraft = async (id, changes, { now = Date.now(), ttlMs } = {}) => {
  ...
  validateSerializable(nextPayload);
  validateSerializable(nextMetadata, 'metadata');

  const nextExpiresAt = ttlMs === undefined ? existing.expiresAt : now + ttlMs;
  const updated = {
    ...existing,
    ...changes,
    id: existing.id,
    ...
    payload: cloneDraft(nextPayload),
    metadata: cloneDraft(nextMetadata),
  };
  assertDraft(updated);

  const db = await getDb();
  await db.table(TABLE_NAME).put(updated);
...
export const listDrafts = async ({ workflow, now = Date.now(), cleanupExpired = true } = {}) => {
  ...
  const drafts = workflow
    ? await db.table(TABLE_NAME).where('workflow').equals(workflow).toArray()
    : await db.table(TABLE_NAME).toArray();
  const valid = [];
...
export const clearExpiredDrafts = async ({ now = Date.now() } = {}) => {
  const db = await getDb();
  const drafts = await db.table(TABLE_NAME).toArray();
  const expiredIds = drafts.filter((draft) => {
    try {
      assertDraft(draft);
      return isExpired(draft, now);
    } catch {
      return true;
    }
  }).map((draft) => draft.id);
...
export const clearAllDraftsForTests = async () => {
  const db = await getDb();
...
export const createDebouncedDraftSaver = (save, delayMs = 500) => {
  ...
  const schedule = (value) => {
    ...
    timer = setTimeout(async () => {
      const valueToSave = pending;
      pending = null;
      timer = null;
      try { await save(valueToSave); } catch (error) { console.warn('[AdminDraft] Debounced save failed', error); }
    }, delayMs);
  };
